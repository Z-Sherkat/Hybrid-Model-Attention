...
from tensorflow.keras.layers import Dropout, LeakyReLU, TimeDistributed
...
def create_lstm(optimizations,latent_dimension,loss):
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(unit, activation='relu', return_sequences=False), input_shape=(train_X.shape[1], train_X.shape[2])))
    model.add(Dense(1))
...
def Optimize(opts,dim,loss,batch=512,epoch=515,verbose=0):
    ...
    plt.savefig('BiLSTM-1layer-dim: '+str(dim)+' loss: '+str(loss)+' opt: '+str(opts)+'-1.png') 
    plt.close()
    return temp_result    
